...
export const MedBlog: React.FC = () => {
  ...
  const allTags = Array.from(new Set(MOCK_POSTS.flatMap(p => p.metadata.tags)));

  const now = new Date('2026-03-07');

  let filteredPosts = MOCK_POSTS.filter(p => {
  ...
  if (sortByDate === 'newest') {
    filteredPosts = [...filteredPosts].sort((a, b) => new Date(b.metadata.publishedDate).getTime() - new Date(a.metadata.publishedDate).getTime());
  } else if (sortByDate === 'oldest') {
    filteredPosts = [...filteredPosts].sort((a, b) => new Date(a.metadata.publishedDate).getTime() - new Date(b.metadata.publishedDate).getTime());
  }
          ...
          <div className="flex-1 p-8 md:p-12 lg:p-16 relative z-10 text-white text-center lg:text-left">
            ...
            >
              <h1 className="text-3xl md:text-5xl lg:text-6xl font-display font-bold mb-6">
                Med-<span className="text-white/80">Blog</span> & Research
              </h1>
              <p className="text-base md:text-lg text-white/90 max-w-lg mb-8 mx-auto lg:mx-0">
                Stay at the forefront of medical science with our expert-authored articles, research summaries, and clinical updates.
              </p>
            </motion.div>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            <button
              ...
              className={clsx(
                ...
                !selectedTag ? "bg-blog-primary text-white" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-200"
              )}
            >
              All
            </button>
            {allTags.map(tag => (
              <button
                ...
                className={clsx(
                  "px-6 py-2 rounded-full text-sm font-bold transition-all",
                  selectedTag === tag ? "bg-blog-primary text-white" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-200"
                )}
              >
                {tag}
              </button>
          ...
          <div className="flex flex-wrap gap-3">
            {[
              ...
              <button
                ...
                className={clsx(
                  "px-6 py-2 rounded-full text-sm font-bold transition-all",
                  dateFilter === period.id ? "bg-blog-primary text-white" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-200"
                )}
              >
                {period.label}
              </button>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3">
            <button
              ...
              className={clsx(
                "px-6 py-2 rounded-full text-sm font-bold transition-all",
                sortByDate === 'newest' ? "bg-blog-primary text-white" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-200"
              )}
            >
              Newest First
            </button>
            <button
              ...
              className={clsx(
                "px-6 py-2 rounded-full text-sm font-bold transition-all",
                sortByDate === 'oldest' ? "bg-blog-primary text-white" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-200"
              )}
            >
              Oldest First
            </button>
